...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  async getInventorySyncSettingsForStore(client: PoolClient, storeId: number): Promise<InventorySyncSettings> {
  ...
  async occupyDiningTable(client: PoolClient, user: AuthenticatedUser, tableName: string | null | undefined, partySize: number) {
  ...
  async releaseDiningTable(client: PoolClient, user: AuthenticatedUser, tableName: string | null | undefined, partySize: number) {
    const tableNumber = this.tableNumberFromName(tableName);
  ...
  async stopOrderRunningTimer(client: PoolClient, orderId: number) {
    await this.queryWithClient(
      client,
      `
        UPDATE orders
   ...
   */
  async reconcileRestaurantRunningTimers(user: AuthenticatedUser) {
  ...
  async validateRestaurantModifiers(client: PoolClient, storeId: number, item: any) {
  ...
  async recordRestaurantIngredientCustomizations(client: PoolClient, storeId: number, orderItemId: number, item: any) {
  ...
  async deductRestaurantIngredients(
    client: PoolClient,
    storeId: number,
    orderId: number,
    orderItemId: number,
  ...
  async writeInventorySaleRecords(
    client: PoolClient,
  ...
  async createUniqueOrderNumber(client: PoolClient, requestedOrderNumber: unknown) {
  ...
  async createUniquePaymentNumber(client: PoolClient, requestedPaymentNumber: unknown) {
  ...
  handleDatabaseWriteError(error: unknown, fallbackMessage: string): never {
