...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@inventory': path.resolve(__dirname, '../../inventory/frontend/src'),
  ...
  server: {
    fs: {
      allow: [
        ...
        path.resolve(__dirname, '../../inventory/frontend/src'),
      ],
    },
    proxy: {
      '/api': {
        target: process.env.VITE_INVENTORY_API_BASE_URL ?? 'http://localhost:3001',
        changeOrigin: true,
      },
    },
  },
